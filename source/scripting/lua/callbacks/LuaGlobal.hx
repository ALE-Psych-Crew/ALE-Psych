...
class LuaGlobal extends LuaPresetBase
{
    override public function new(lua:LuaScript)
    {
        ...
        if (type == SUBSTATE)
        {
            set('close', function()
            {
                (cast (game, ScriptedSubState)).close();
            });
        }

        set('switchToCustomState', function(name:String, ?hsArguments:Array<Dynamic>, ?luaArguments:Array<Dynamic>)
        {
            CoolUtil.switchState(new CustomState(name, hsArguments, luaArguments));
        });

        set('openCustomSubState', function(name:String, ?hsArguments:Array<Dynamic>, ?luaArguments:Array<Dynamic>)
        {
            CoolUtil.openSubState(new CustomSubState(name, hsArguments, luaArguments));
        });
    }
