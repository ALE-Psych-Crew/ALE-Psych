...
import core.enums.VisualizerType;
import core.structures.VisualizerAudioInfo;
import lime.utils.Int16Array;
import funkin.audio.Analyzer;
...
class SpectogramSprite extends FlxSpriteGroup
{
  ...
  public var visType:VisualizerType = UPDATED;

  public var col:Int = FlxColor.WHITE;
  public var size:Float = FlxG.height;

  public var orientation:Orientation;

  public var vis:Analyzer;

  public function new(daSound:FlxSound, ?size:Float, ?col:FlxColor = FlxColor.WHITE, ?orientation:Orientation, ?amnt:Int = 500)
  {
    super();

    vis = new Analyzer(daSound);
    this.col = col;
    this.size = size ?? FlxG.width;
  ...
  public function regenLineShit():Void
  {
    ...
    {
      final position = {
        x: i / lengthOfShit * size,
        y: 100
      }

      var lineShit:FlxSprite = new FlxSprite(orientation == HORIZONTAL ? position.x : position.y, orientation == HORIZONTAL ? position.y : position.x).makeGraphic(1, 1, col);
  ...
  public function generateSection(start:Float = 0, seconds:Float = 1):Void
  {
    ...
    if (setBuffer)
    {
      ...
      for (i in 0...group.members.length)
      {
        ...
        var curAud:VisualizerAudioInfo = Analyzer.getCurAud(audioData, sampleApprox);
        ...
        final position = {
          x: (i / group.members.length * size) + y,
          y: (curAud.balanced * swagheight / 2 + swagheight / 2) + x
        };

        prevLine.x = orientation == HORIZONTAL ? position.x : position.y;
        prevLine.y = orientation == HORIZONTAL ? position.y : position.x;
  ...
  public function updateFFT()
  {
    if (vis.snd != null)
    {
      ...
      if (setBuffer && doAnim)
      {
        ...
        {
          var curAud:VisualizerAudioInfo = Analyzer.getCurAud(audioData, i);
          i += 2;
        ...
        for (i in 0...group.members.length)
        {
          ...
          final position = {
            x: (i / group.members.length * size) + y,
            y: (freqIDK * swagheight / 2 + swagheight / 2) + x
          }
    ...
    if (vis.snd != null)
    {
      ...
      if (setBuffer)
      {
        ...
        for (sample in remappedShit...remappedShit + lengthOfShit)
        {
          ...
          final position = {
            x: (Std.int(remappedSample) / lengthOfShit * size) + y,
            y: (curAud.balanced * swagheight / 2 + swagheight / 2) + x
          };
