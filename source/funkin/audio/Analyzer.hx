...
class Analyzer
{
  ...
  public function funnyFFT(samples:Array<Float>, ?skipped:Int = 1):Array<Array<Float>>
  {
    ...
    {
      var powShit:Float = FlxMath.remapToRange(k, 0, halfN, 0, CoolUtil.logBase(10, halfN)); // 4.3 is almost the log of 20Khz or so. Close enuf lol
    ...
    while (c < samples.length)
    {
      ...
      final freqs = FastFourierTransform.rfft(chunk).map(z -> z.scale(1 / fftN).magnitude).mapi(melodicBandPass);
  ...
  public static function getCurAud(aud:Int16Array, index:Int):VisualizerAudioInfo
  {
    var left = aud[index] / 32767;
    var right = aud[index + 2] / 32767;
    var balanced = (left + right) / 2;

    var funny:VisualizerAudioInfo = {left: left, right: right, balanced: balanced};
